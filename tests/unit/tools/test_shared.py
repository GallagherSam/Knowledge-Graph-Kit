...
def test_search_nodes(shared_instance, mock_crud, mock_db_session):
    ...
    mock_crud.search_nodes.assert_called_once_with(
        db=mock_db_session, query="test", node_type="Note", tags=["tag1"]
    )
...
def test_delete_node(shared_instance, mock_crud, mock_db_session, mock_vector_store_instance):
    ...
    mock_crud.delete_node.assert_called_once_with(
        db=mock_db_session, vector_store=mock_vector_store_instance, node_id="test_id"
    )
