...
def test_update_note_integration(tools_instance):
    ...
    updated_note = tools_instance.notes.update_note(
        note_id=note_id, title=new_title, tags=new_tags
    )
...
def test_node_search_integration(tools_instance):
    ...
    note1 = tools_instance.notes.create_note(
        title="Unique Apple Note",
        content="Content about oranges.",
        tags=["fruit", "food"]
    )
    note2 = tools_instance.notes.create_note(
        title="Another Note",
        content="This one is about a unique banana.",
        tags=["fruit", "yellow"]
    )
    note3 = tools_instance.notes.create_note(
        title="Third Note",
...
def test_node_semantic_search_integration(tools_instance):
    ...
    note_fruit_1 = tools_instance.notes.create_note(
        title="Fruit Basket",
        content="Apples, bananas, and oranges are great for a balanced diet."
    )
    note_fruit_2 = tools_instance.notes.create_note(
        title="Tropical Tastes",
        content="Mangoes and pineapples are delicious tropical fruits."
    )
    note_tech = tools_instance.notes.create_note(
        title="Software Development",
        content="Building applications with Python and JavaScript."
    )
    # Also create a non-Note node to ensure it's filtered out
    task_unrelated = tools_instance.tasks.create_task(
        description="File the TPS reports."
    )
